...
$order_by2='asc';
$where=" and tbl_document.status=0";
$sql=executeQuery("select tbl_document.*,tbl_user.username from tbl_document inner join tbl_user on (tbl_document.posted_by=tbl_user.id) where 1=1 $where order by $order_by $order_by2 limit $start,$pagesize");
