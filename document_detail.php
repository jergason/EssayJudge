...
if($_GET[plus_score]==1)
{
  if($_SESSION[uid])
  {
    ...
    $chk_comment_posted=mysql_num_rows(mysql_query("select id from tbl_comment_chk where comment_id=".$_GET[comment_id]." and user_id=".$_SESSION[uid]));
    ...
    executeUpdate("insert into tbl_comment_chk set comment_id=".$_GET[comment_id].",user_id=".$_SESSION[uid]);
...
<br>   
<center>
<span class="essayTitle"><?php echo ucwords($line[doc_title]);?></span></center>
<br />

<br>
<br>
            <div class="essaydisp"><div class="essaycont"><?php echo nl2br(nl2br($line[doc_content]));?>

<br><br><center><small>Submitted by:</small><span class="essayAuthor"><?php echo $line[username];?></span></center>
               <? $authorname = $line[username]; $authorid = $line[user_id]; ?>
              </div></div>
                  <br /><div style="margin-left:20px"><a href="javascript:void(0);" style="text-decoration:none;text-style:oblique" onClick="open_post_form('k_list');">&lt;Keywords&gt;</a><span id="k_list" style="display:none">&nbsp;&nbsp;&nbsp;<?php echo $line[keywords]?></span></div>
                  <br />
...
<? for($k = 0; $line=mysql_fetch_array($sql); $k++)
{
?>
<!--ONE MSG BEGINS-->
                  <tr>
                     <td class="commentUser" style="text-align:center;background-color:<? if($k%2==0){echo '#F2F2F2';}else{echo '#D2D2D2';}?>;padding-top:7px;border-bottom:1px solid #666666;border-right:1px solid #666666" rowspan=2>
<? if ($authorid==$line[user_id]){ ?>
<i>Author:</i><br>
<? } ?>
                        <a href="user_profile.php?user_id=<?=$line[user_id]?>" style="line-height:28px;font-size:12px"><?php echo ucwords($line[username]);?></a>
<br />
...
$rs_comment=mysql_fetch_array($sql_comment);?>
                        ...
                        <br /><span style="font-weight:bolder;font-style:italic;margin-left:-25px;font-size:10px">Member Since</span><div class="bgstatspace nodec" style="font-size:10px;margin-left:5px;font-style:italic"><?echo $line[join_date] ?></div>
                     </td>
                     <td class="commentMsg" style="padding:5px;background-color:<? if($k%2==0){echo '#F2F2F2';}else{echo '#D2D2D2';}?>;font-size:10px;">
                        <?php echo ucfirst(nl2br($line[comment]));?>
                     </td>
                  </tr>
                  <tr>
                     <td style="padding:5px;vertical-align:bottom;background-color:<? if($k%2==0){echo '#F2F2F2';}else{echo '#D2D2D2';}?>;font-size:10px;border-bottom:1px solid #666666">
...
$result_comment1=mysql_num_rows($comment_query1); }?>
                        <?php if($result_comment1==0){?>                        Rate Comment: <a href="document_detail.php?minus_score=1&comment_id=<?=$line[id]?>&doc_id=<?=$_GET[doc_id]?>"><img src="images/minuscomment.gif" style="border:0;margin-left:2px"></a><a href="document_detail.php?plus_score=1&comment_id=<?=$line[id]?>&doc_id=<?=$_GET[doc_id]?>"><img src="images/pluscomment.gif" style="border:0;margin-left:4px"></a>
...
<?$line=mysql_fetch_array(mysql_query("select tbl_document.id,tbl_document.doc_title,tbl_document.keywords,date_format(tbl_user.join_date,'%b %D, %Y') as join_date,date_format(tbl_user.last_activity,'%b %D, %Y') as last_activity, tbl_document.doc_title,date_format(tbl_document.post_date,'%b %D, %Y') as postDate, tbl_document.post_date,tbl_document.doc_path,tbl_document.doc_content,tbl_user.username,tbl_user.id as user_id,count(distinct tbl_comment.id) as total_comment,sum( distinct tbl_comment.score) as comment_point,count( distinct tbl_rating.score) as numVotes,avg( distinct tbl_rating.score) as avg_score from tbl_document inner join tbl_user on (tbl_document.posted_by=tbl_user.id) left join tbl_comment on (tbl_document.id=tbl_comment.doc_id) left join tbl_rating on (tbl_document.id=tbl_rating.document_id) where 1=1 and tbl_document.id='$_GET[doc_id]'  group by tbl_document.id"));?>
<td>
